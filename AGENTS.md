...
cd programs/player-economy && cargo fmt

# Run Rust linting
cd programs/hero-core && cargo clippy
cd programs/player-economy && cargo clippy
```
...
- Item management: Items found during adventures are stored in `ItemSlot` array (max 6 slots) and can be picked up, dropped, or swapped
...
- VRF callback pattern: `mint_dungeon` requests randomness, `callback_mint_dungeon` consumes it. Same pattern for hero minting and stat rerolling.
...
- Item management: `player-economy` stores persistent items, `adventure-engine` stores ephemeral adventure items that can be moved to persistent storage
...
### Client Testing
- Web logic relies on `vitest`; place new unit specs next to the source with a `.test.ts` suffix
- Run tests with `yarn app:test`
- Aim for deterministic tests: seed RNG-dependent systems

### On-Chain Testing
- Anchor integration suites live under `tests/` and run through `ts-mocha` via `anchor test`
- Create files such as `tests/new_feature.test.ts`
- Load fixtures from `tests/fixtures/`
- Clean up created accounts after tests
- Test VRF callback patterns thoroughly

## Key Game Mechanics

### Hero System
- Heroes have stats: HP, Attack, Defense, Magic, Resistance, Speed, Luck
- Heroes accumulate stress during adventures (affects performance)
- Positive and negative traits modify hero behavior
- Status effects can be applied during combat
- Heroes gain XP and level up, improving stats
- Town services manage hero wellbeing between adventures

### Economy System
- Players earn gold through adventures and hourly grants
- Items have buy/sell prices (some items are sell-only drops)
- Stack limits prevent infinite item accumulation
- Seven item types with different rarities and uses
- Economy is persistent across all adventures

### Adventure System
- Adventures are ephemeral (fast, gasless transactions on rollup)
- Heroes are locked during active adventures
- Party can have up to 4 heroes
- Items collected during adventure stored separately from persistent inventory
- Adventures can be reset daily
- Portal system allows dungeon traversal

### Dungeon Generation
- Dungeons are generated deterministically from seed
- Grid-based layout with rooms and corridors
- Chests, doors, and portals placed algorithmically
- Client and on-chain logic must stay synchronized
- Use VRF for unpredictable dungeon seeds

## Important Patterns

### VRF Integration
- Request randomness in initial instruction (e.g., `mint_dungeon`)
- Consume randomness in callback instruction (e.g., `callback_mint_dungeon`)
- Store VRF request state in account
- Never rely on client-side randomness for critical game state

### Cross-Program Invocation (CPI)
- `adventure-engine` calls `hero-core` for hero locking/unlocking
- `adventure-engine` calls `hero-core` for HP/XP updates
- Always validate account ownership in CPI calls
- Use proper seeds and bumps for PDA derivation

### Ephemeral Rollups
- Use `#[delegate]` macro for delegation setup
- Use `#[ephemeral]` macro for fast, gasless operations
- Use `#[commit]` macro for committing state back to L1
- Understand MagicBlock's delegation model for proper implementation
...
│   ├── logic.rs            # Game logic (if applicable)
│   ├── helpers.rs          # Helper functions (if applicable)
│   └── instructions/
│       ├── mod.rs          # Instruction module declarations
│       ├── mint.rs         # Minting instructions
│       ├── config.rs       # Configuration instructions
│       └── [feature].rs    # Feature-specific instructions
└── Cargo.toml
```
...
│   ├── state/              # State management and chain interaction
...
1. Define instruction in `programs/[name]/src/lib.rs`
2. Create handler in `programs/[name]/src/instructions/[feature].rs`
3. Add accounts context struct
4. Implement validation and business logic
5. Add error cases to `errors.rs`
6. Run `anchor build` to regenerate IDL
7. Add client-side function in `app/src/state/[program]Chain.ts`
8. Write integration test in `tests/`

### Adding a New Item Type
1. Update `ITEM_COUNT` in `programs/player-economy/src/constants.rs`
2. Add new variant to `ItemKey` enum in `state.rs`
3. Add item definition to `ITEM_DEFINITIONS` array
4. Update client types and UI to display new item
5. Test buy/sell/consume flows

### Modifying Hero Stats
1. Update stat fields in `programs/hero-core/src/state.rs`
2. Adjust size calculations in account `LEN` constants
3. Update snapshot logic in `adventure-engine`
4. Regenerate types with `anchor build`
5. Update client display logic

## Documentation and Resources

- `docs/` - Gameplay and integration documentation
- `docs/magicblock/` - Full MagicBlock ephemeral rollup documentation
...
**IDL out of sync:**
- Run `anchor build` to regenerate IDL and types
- Restart TypeScript server in your editor
- Clear `target/` and rebuild if types are still stale

**Account size mismatch:**
- Verify account space calculations in `state.rs`
- Check that all fields are accounted for in `LEN` constants
- Remember discriminator (8 bytes) and vector prefixes (4 bytes)

**VRF callback failing:**
- Ensure callback instruction includes all required accounts
- Verify VRF request state is properly stored
- Check Orao VRF program is included in accounts

**Delegation not working:**
- Verify World program is properly configured
- Check delegation macros are applied correctly
- Ensure ephemeral account is properly initialized
